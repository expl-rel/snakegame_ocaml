...
let handle_key t key =
  ...
  (* let () = set_direction t.snake {of_key key} in *)
  let key = Direction.of_key key in
  match key with
  | None -> ()
...
let check_for_collisions t =
  ...
  let inbound= Board.in_bounds t.board (Snake.head t.snake) in
  match inbound with
...
(* Exercise 04b:
   ...
   (* step is false if snake collies *)
let step t =
